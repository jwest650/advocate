...
export const LanguageSwitcher: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isDemoMode()) {
            ...
            if (cookieLang) {
                const cookieLanguage = languages.find((l: Language) => l.code === cookieLang);
                if (cookieLanguage) {
                    setCurrentLanguage(cookieLanguage);
                    // Force i18n to change to cookie language if different
    ...
    const handleLanguageChange = async (languageCode: string) => {
        ...
        if (lang) {
            ...
            try {
                ...
                // Update layout position through context
                updatePosition(newDirection as 'left' | 'right');

                // Save language and layout preferences
                if (isDemoMode()) {
                    setCookie('app_language', languageCode);
                    setCookie('layoutPosition', newDirection);
                } else {
                    // Also save to localStorage for i18next persistence
                ...
                // Dispatch events for layout update
